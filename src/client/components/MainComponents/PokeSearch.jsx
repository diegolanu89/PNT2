	...
	const controladorService = (pagina) => {
		fetch(pagina)
			...
			.then((pokeData) => {
				...
				console.log(results)
				const userPromises = results.map((urlDetail) => fetch(urlDetail.url).then((res) => res.json()))
				return Promise.all(userPromises).then((pokemonesDetail) => ({ results, pokemonesDetail }))
			})
			.then(({ pokemonesDetail }) => {
				console.log('DETALLES:', pokemonesDetail)
			})
			.catch((error) => {
