...
export default function AppBarPokemon() {
	const [anchorEl, setAnchorEl] = useState(null) // Estado para el menú hamburguesa
		...
		<Box sx={{ flexGrow: 1 }}>
			<AppBar position="static">
				<Toolbar>
					{user && (
						<>
							...
							<Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
								...
								<MenuItem onClick={handleLogout}>CerrarSesion</MenuItem>
							</Menu>
						</>
					)}
					...
					<Box>{user ? <Avatar alt={user.displayName || 'Usuario'} src={user.photoURL || '/pika.png'} /> : <Avatar />}</Box>
				</Toolbar>
			</AppBar>
		</Box>
	)
